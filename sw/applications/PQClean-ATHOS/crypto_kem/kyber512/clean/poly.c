...
void PQCLEAN_KYBER512_CLEAN_poly_getnoise_eta1(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce) {
    ...
    //unsigned int cycles_cbd3;
    
    //CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1);
    // Starting the performance counter
    //CSR_WRITE(CSR_REG_MCYCLE, 0);
    PQCLEAN_KYBER512_CLEAN_poly_cbd_eta1(r, buf);
    //CSR_READ(CSR_REG_MCYCLE, &cycles_cbd3);
    //printf("Number of clock cycles PQCLEAN_KYBER512_CLEAN_poly_cbd_eta3: %d\n", cycles_cbd3);
}
...
void PQCLEAN_KYBER512_CLEAN_poly_tomont(poly *r) {
    ...
    for (i = 0; i < KYBER_N; i++) {
        ...
        //CSR_READ(CSR_REG_MCYCLE, &cycles_montg);
        //printf("Number of clock cycles PQCLEAN_KYBER512_CLEAN_montgomery_reduce: %d\n", cycles_montg);
