...
/* Code to generate PQCLEAN_KYBER51290S_CLEAN_zetas and zetas_inv used in the number-theoretic transform:
...
void init_ntt() {
  ...
  for(i=0;i<128;i++) {
    ...
    if(PQCLEAN_KYBER51290S_CLEAN_zetas[i] > KYBER_Q/2)
      PQCLEAN_KYBER51290S_CLEAN_zetas[i] -= KYBER_Q;
    if(PQCLEAN_KYBER51290S_CLEAN_zetas[i] < -KYBER_Q/2)
      PQCLEAN_KYBER51290S_CLEAN_zetas[i] += KYBER_Q;
  }
}
